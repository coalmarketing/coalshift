...
Relevant source areas include `app/gdpr`, `app/cookies`, `app/reference`, `app/registrace`, `app/wait-list`, `app/layout.tsx` and their components. The inspected baseline has a global homepage canonical, no per-route metadata, duplicate heading/ID relationships and old trial/launch language. Verify which issues are already resolved by preceding phases before changing them again.

## Scope and implementation steps

- [ ] During planning, record the agreed indexing treatment for registration and historical pages. Keep all seven content routes available; do not infer noindex from lack of a homepage link.
- [ ] Preserve the phase-02 retirement of `/zdravotnici`: keep its 301 to `/#industries`, exclude it from active page metadata/sitemap and do not recreate its body or video. The seven content routes are listed in brief.md.
...
- [ ] Resolve remaining page headings, unique anchors, landmark/label relationships, image alt text and accessible interactive controls.
...
- **AC-02:** Q-011 passes for all edited page bodies/metadata, including lowercase brand spelling, the 14-day trial, careful legal wording and truthful follow-up copy.
...
Jakub and Codex must settle the indexing treatment of historical/registration routes during this phase's planning review. Preserve the seven content routes regardless of that indexing decision. Healthcare retirement and monthly-only pricing were already decided by the owner in phase 02; they are not open choices for this phase.

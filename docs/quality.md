...
| Q-004 | Header/CTA destinations are correct: desktop shows only theme toggle + `Přihlásit se`, no trial CTA anywhere; hero/practical-browser/contact CTAs scroll to `#pricing`; pricing cards link to `REGISTER_URL`. Every `LOGIN_URL`/`REGISTER_URL` CTA (desktop + mobile-menu header login, footer login, all five pricing CTAs) opens in a new tab (`target="_blank"`, `rel` includes `noopener`) with an unchanged visible label and an accessible name warning of the new tab | PASS (logic + semantics + new-tab/`rel`/accessible-name check, local); NOT_RUN (real mobile pointer/keyboard, completed smooth-scroll on a device) | |

...
| Q-017 | Microsoft Bookings opens through a lazy accessible dialog with a direct-link fallback; the YouTube walkthrough uses a click-to-play privacy-enhanced embed with a direct-link fallback; neither is contacted before its user action | PASS (local mechanics and production Bookings-iframe render both confirmed) | |

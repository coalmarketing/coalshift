...
| Faux browser (`FunctionsBrowser.tsx`) | `src/_includes/components/desktop-screen.njk`; `src/assets/js/screen-url-updater.js` | Decorative chrome (not an iframe), centered address pill, always-vertical icon tab rail, one visible panel, three titled feature descriptions (icon + bold title + muted line, `sm:grid-cols-2`) and two supporting numerical cards per topic (phone removed; the panel CTA scrolls to `#pricing`). Real WAI-ARIA `tablist`/`tab`/`tabpanel`, roving focus, `aria-orientation="vertical"` at every width. Address pill is plain display text synced to the selected tab. |

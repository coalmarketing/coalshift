...
**coalsoft blue `#00B5E2`**, with light and dark themes (first visit follows the
system `prefers-color-scheme`) and the coalfamily brand strip. coalios orange is
*not* coalshift's primary color.
...
| Faux browser (`FunctionsBrowser.tsx`) | `src/_includes/components/desktop-screen.njk`; `src/assets/js/screen-url-updater.js` | Decorative chrome (not an iframe), centered address pill, always-vertical icon tab rail, one visible panel, three titled feature descriptions (icon + bold title + muted line, `sm:grid-cols-2`) and two supporting numerical cards per topic (phone removed; Phase 06 replaced the five topics/values and the panel CTA now scrolls to `#pricing`). Real WAI-ARIA `tablist`/`tab`/`tabpanel`, roving focus, `aria-orientation="vertical"` at every width. Address pill is plain display text synced to the selected tab. |
...
| Product gallery — `home/ProductGallery.tsx` | `src/_includes/sections/page-content.njk` | On `xl`: gallery-column left, text/CTA right (`flex-col xl:flex-row`, `xl:basis-1/2`, `xl:order-*` swap so on narrow screens the order is text/CTA/selector → active gallery-column content → prev/next + counter, screenshot mode only). Fluid gap; `.glow-border--lg` (2rem radius) layered rounded frame. Phase 07: a horizontal two-option WAI-ARIA manual-activation tablist (`ModeTabs`, same roving-focus/Enter-Space model as `FunctionsBrowser`'s vertical tabs, each label prefixed with a decorative icon ported byte-for-byte from coalios `play_arrow.svg`/`devices.svg`) sits in the text column and switches between two real, permanently-mounted `role="tabpanel"` elements (own ids/`aria-labelledby`) for **Video ukázka** (default) and **Obrazovky aplikace**, stacked in one CSS-grid overlap "media stage" (`col-start-1 row-start-1`) so the stage height is the grid's natural max of both panels — switching never shifts the section or anything below it, with no JS measurement. The inactive panel crossfades out (opacity + small translate, ~220ms, instant under reduced motion) and is `aria-hidden` + `inert` + `pointer-events-none`. Video mode wraps a `youtube-nocookie.com` iframe in the same `.glow-border--lg` frame as the screenshot cards, behind a first-party click-to-play facade over the locally stored YouTube poster (`aspect-[16/9]`); the screenshot stack below is otherwise unchanged. The coalios stacked-card look uses the reference's real layered-image principle as a **playful, moving** stack: `SLIDES` is rendered in stable order as three persistent cards (each keyed by its screenshot, keeping its own image for the component's life; same rounded frame + clipped corners; `pointer-events-none`, only the front card exposes its `alt`, the rest `aria-hidden`). Each card's role — `front` (straight, readable) / `upper` (above/left) / `lower` (below/right) — comes from the active index; `.pg-card--{role}` in globals.css supplies a lightweight 2D `transform` (`translate` ±3–4% / ±8–12%, `rotate` ∓3°, `scale` .95) plus `z-index`, and only the `transform` is transitioned (~340 ms `cubic-bezier(0.4,0,0.2,1)`). So navigating **moves the actual cards**: `next` cycles upper→front→lower→upper, `previous` reverses; the promoted card takes `z-index: 20` immediately so it rises cleanly, no image swaps, interrupted transitions retarget from the current position (no queue). `prefers-reduced-motion` drops the transition (roles snap). The tilted cards do translate horizontally (±4%, part of the same `transform`); generous wrapper padding (`px-8 pb-14 pt-12 …`) plus the tuned transform/rotation/scale constants keep them within the reserved space at the tested widths → no page-level horizontal overflow and the controls sit clear below. A single transparent `<button>` overlay (z-30, `aspect-[2876/1376]`, `cursor-zoom-in`) at the front-card box is the one stable keyboard focus target and fullscreen trigger — the moving cards are never tab stops; there is no separate fullscreen icon. Keeps the native 2876×1376 ratio (no `object-cover`/`aspect-[3/2]` — never crop the app UI). The fullscreen `role="dialog"` keeps its own accepted keyed slide-in (`.pg-slide` + `.pg-slide-next` / `.pg-slide-prev`, 200 ms), unchanged. Deliberate previous/next + `1 / 3` counter, wrap navigation, no autoplay, `touch-action: pan-y` horizontal swipe (a swipe suppresses the follow-up click so it never opens fullscreen). See [content-and-seo.md](content-and-seo.md) §"Product gallery". |
...
| Team/contact cards (`Contact.tsx`) | `src/_includes/components/tym-item.njk`; `sections/our-team.njk` | Phase 07: each person is one cohesive `.glow-border` card — round portrait (64–80px) beside name/role/contact rows inside a single inner surface wrapper (a `.glow-border`'s `> *` selector styles every direct child as its own rounded surface, so a card must keep exactly one direct child or it reads as detached slabs). Two cards, confirmed contact methods only, no invented social links. Desktop lays these out in the left one-third of a 3-column grid (`xl:grid-cols-3`) beside a right two-thirds `.glow-border--lg` consultation panel; mobile stacks the consultation panel first via `order-1`/`order-2`, contacts `order-2`/`xl:order-1`. Same `SpotlightGroup` now spans all three cards. |
| Consultation panel + Bookings dialog (`Contact.tsx` + `BookingsDialog.tsx`, Phase 07) | owner request (Microsoft Bookings, external) | One unified `.glow-border--lg` card (single inner surface — same one-direct-child rule as the contact cards) matching the browser-frame/gallery radius, stretching to the contacts column's height (`xl:items-stretch`); eyebrow/heading/body as one content block, CTA + "Online přes Microsoft Teams" anchored near the bottom via `mt-auto`, and a restrained coalshift-blue radial-gradient wash (`bg-[radial-gradient(...)]`, no separate DOM node, does not touch text contrast) filling the stretch space. CTA opens a first-party dialog reusing `ProductGallery`'s fullscreen-dialog contract (focus trap via Tab-wrap, Escape close, body scroll lock, background `inert`, focus restored to the opener) sized `sm:max-w-2xl sm:h-[85vh]` (contained) / full-viewport below `sm` (mobile). The cross-origin Bookings document is never restyled/scripted; a permanent direct-link fallback sits below the iframe. |
...
`darkMode: 'selector'` — the render-blocking bootstrap in `app/layout.tsx`
(`themeScript.ts`) sets the `dark` class, `color-scheme` and `data-theme` on
`<html>` before paint. Resolution order: a valid `light`/`dark` value in
`localStorage['coalshift-theme']` (a manual choice) → the system
`prefers-color-scheme` → a safe light fallback if storage or `matchMedia` is
unavailable. `ThemeToggle` writes the explicit choice, so a manual selection
keeps overriding the system across routes and reloads; it stays safe when
storage is blocked and has a stable accessible name plus `aria-pressed` for the
dark state. A later OS change on an already-open page is not followed.
...
| Header | Desktop nav items `h-20` (80px); Inter nav text 16px; desktop nav from `xl` (1280px). Family bar `h-11` (~44–48px), marks `size-7` (28px). Scroll threshold 100px. Nav bar `h-[4.5rem]` (72px) below `xl`, `h-20` (80px) from `xl`. `--header-height` = nav-bar box height (content + 1px top/bottom border), CSS-owned default `calc(4.5rem + 2px)` / `calc(5rem + 2px)` at `xl`, republished as the measured px after mount; `--family-block-h` (default `4rem` = pt-3 + h-11 + pb-2) drives the scroll translate and the document-flow spacer `calc(--family-block-h + --header-height)`. |
